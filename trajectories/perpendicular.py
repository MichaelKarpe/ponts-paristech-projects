...
class Perpendicular(Trajectory):
    def __init__(self, white_ball, coords_balls, index_ball, index_pos, verbose=True):
        ...
        self.directions = ["left_to_up", "up_to_left", "up_to_right", "right_to_up",
                           "right_to_down", "down_to_right", "down_to_left", "left_to_down"]

        self.opposite = {"left_to_up": self.white_ball.x - self.hole_width,
        ...
        self.adjacent = {"left_to_up": self.white_ball.y - self.radius,
        ...
        self.correct_opposite = {"left_to_up": self.white_ball.x - self.radius,
        ...
        self.correct_adjacent = {"left_to_up": self.white_ball.y - self.radius,
                                 "up_to_left": self.white_ball.x - self.radius,
                                 "up_to_right": self.white_ball.x - self.radius,
                                 "right_to_up": self.width - self.white_ball.y - self.radius,
                                 "right_to_down": self.width - self.white_ball.y - self.radius,
    ...
    def reachable_zone(self, direction):
        ...
        for direction in self.directions:
            ...
            if alpha[direction] <= np.pi / 4:
                ...
                correct_alpha[direction] = np.arctan(self.correct_opposite[direction]
                                                     / (self.correct_adjacent[
                                                            direction] + self.hole_width - self.radius))

        plots = {"left_to_up": Point(self.white_ball.x - 2 * self.radius * np.sin(correct_alpha["left_to_up"]),
                                     self.white_ball.y - 2 * self.radius * np.cos(correct_alpha["left_to_up"])),
                 "up_to_left": Point(self.white_ball.x - 2 * self.radius * np.cos(correct_alpha["up_to_left"]),
                                     self.white_ball.y - 2 * self.radius * np.sin(correct_alpha["up_to_left"])),
                 "up_to_right": Point(self.white_ball.x - 2 * self.radius * np.cos(correct_alpha["up_to_right"]),
                                      self.white_ball.y + 2 * self.radius * np.sin(correct_alpha["up_to_right"])),
                 "right_to_up": Point(self.white_ball.x - 2 * self.radius * np.sin(correct_alpha["right_to_up"]),
                                      self.white_ball.y + 2 * self.radius * np.cos(correct_alpha["right_to_up"])),
                 "right_to_down": Point(self.white_ball.x + 2 * self.radius * np.sin(correct_alpha["right_to_down"]),
                                        self.white_ball.y + 2 * self.radius * np.cos(correct_alpha["right_to_down"])),
                 "down_to_right": Point(self.white_ball.x + 2 * self.radius * np.cos(correct_alpha["down_to_right"]),
                                        self.white_ball.y + 2 * self.radius * np.sin(correct_alpha["down_to_right"])),
                 "down_to_left": Point(self.white_ball.x + 2 * self.radius * np.cos(correct_alpha["down_to_left"]),
                                       self.white_ball.y - 2 * self.radius * np.sin(correct_alpha["down_to_left"])),
                 "left_to_down": Point(self.white_ball.x + 2 * self.radius * np.sin(correct_alpha["left_to_down"]),
    ...
    def change_of_basis(self, direction):
        ...
        vectors_balls = [np.array([[self.coords_balls[i].x - first_rebound.x],
                                   [self.coords_balls[i].y - first_rebound.y]])
                         for i in range(self.nb_balls - 1)]

        (u1, u2) = (second_rebound.x - first_rebound.x,
                    second_rebound.y - first_rebound.y)
        ...
        for index_ball in range(self.nb_balls - 1):
            impact_positions = Ball(white_ball=self.white_ball, coords_balls=self.coords_balls,
                                    index_ball=index_ball, verbose=False).impact_positions
            vectors_shocks = [np.array([[impact_positions[index_pos].x - first_rebound.x],
    ...
    def two_rebounds_angle(self, direction):
        ...
        # adjacent
        c = {"left_to_up": self.impact_pos.y - self.radius,
        ...
        # opposite
        d = {"left_to_up": self.impact_pos.x - self.radius,
    ...
    def rebound_positions(self, direction):
        ...
        rebound_coords = {"left_to_up": [Point(self.white_ball.x - b1, self.radius),
                                         Point(self.radius, self.impact_pos.y - c2)],
                          "up_to_left": [Point(self.radius, self.white_ball.y - b1),
                                         Point(self.impact_pos.x - c2, self.radius)],
                          "up_to_right": [Point(self.radius, self.white_ball.y + b1),
                                          Point(self.impact_pos.x - c2, self.width - self.radius)],
                          "right_to_up": [Point(self.white_ball.x - b1, self.width - self.radius),
                                          Point(self.radius, self.impact_pos.y + c2)],
                          "right_to_down": [Point(self.white_ball.x + b1, self.width - self.radius),
                                            Point(self.length - self.radius, self.impact_pos.y + c2)],
                          "down_to_right": [Point(self.length - self.radius, self.white_ball.y + b1),
                                            Point(self.impact_pos.x + c2, self.width - self.radius)],
                          "down_to_left": [Point(self.length - self.radius, self.white_ball.y - b1),
                                           Point(self.impact_pos.x + c2, self.radius)],
                          "left_to_down": [Point(self.white_ball.x + b1, self.radius),
                                           Point(self.length - self.radius, self.impact_pos.y - c2)]}

        distances = [a[direction] / np.cos(alpha),
