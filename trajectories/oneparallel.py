...
class Oneparallel(Trajectory):
    ...
    def rebound_positions(self, direction):
        dist_to_directions = {"x_left": self.white_ball.x + self.impact_pos.x - 2 * self.radius,
        ...
        rebound_coords = {
            "left": [Point(self.radius,
                           ...
                           / abs(self.impact_pos.y - self.white_ball.y) * dist_to_directions["y_left"]
                           * (self.white_ball.x - self.radius) / dist_to_directions["x_left"])],
            "top": [Point(self.white_ball.x + (self.impact_pos.x - self.white_ball.x)
                          / abs(self.impact_pos.x - self.white_ball.x) * dist_to_directions["x_top"]
                          * (self.width - (self.white_ball.y + self.radius)) / dist_to_directions["y_top"],
                          self.width - self.radius)],
            "right": [Point(self.length - self.radius,
                            self.white_ball.y + (self.impact_pos.y - self.white_ball.y)
                            / abs(self.impact_pos.y - self.white_ball.y) * dist_to_directions["y_right"]
                            * (self.length - (self.white_ball.x + self.radius)) / dist_to_directions["x_right"])],
            "bottom": [Point(self.white_ball.x + (self.impact_pos.x - self.white_ball.x)
                             / abs(self.impact_pos.x - self.white_ball.x) * dist_to_directions["x_bottom"]
                             * (self.white_ball.y - self.radius) / dist_to_directions["y_bottom"],
                             self.radius)]
        }
