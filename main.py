...
if __name__ == '__main__':
    ...
    for ind_ball in range(test_billiard.nb_balls-1):
        for ind_pos in range(test_billiard.nb_holes):
            test_direct_trajectory = Directtraj(white_ball=white_ball, coords_balls=coords_balls,
                                                index_ball=ind_ball, index_pos=ind_pos)
            test_oneparallel_trajectory = Oneparallel(white_ball=white_ball, coords_balls=coords_balls,
                                                      index_ball=ind_ball, index_pos=ind_pos)
            test_twoparallel_trajectory = Twoparallel(white_ball=white_ball, coords_balls=coords_balls,
                                                      index_ball=ind_ball, index_pos=ind_pos)
            test_perpendicular_trajectory = Perpendicular(white_ball=white_ball, coords_balls=coords_balls,
                                                          index_ball=ind_ball, index_pos=ind_pos)
    plt.show()
