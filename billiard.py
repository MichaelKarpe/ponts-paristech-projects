...
class Billiard:
    def __init__(self, white_ball=None, coords_balls=None, verbose=True):
        ...
        self.holes = [Point(self.radius, self.radius),
    ...
    def put_random_balls(self, nb_balls):
        ...
        while len(coords_balls) < nb_balls:
            if len(coords_balls) == 0:
                new_coords = Point((self.length - 4 * self.hole_width) * rand() + 2 * self.hole_width,
                                   (self.width - 4 * self.hole_width) * rand() + 2 * self.hole_width)
            else:
                new_coords = Point((self.length - 2 * self.hole_width) * rand() + self.hole_width,
    ...
    def draw_billiard(self):
        holes_borders = {"bottom_left": [Point(0, self.hole_width),
                                         Point(0, 0),
                                         Point(self.hole_width, 0)],
                         "bottom_middle": [Point(self.length / 2 - self.hole_width, 0),
                                           Point(self.length / 2, 0),
                                           Point(self.length / 2 + self.hole_width, 0)],
                         "bottom_right": [Point(self.length - self.hole_width, 0),
                                          Point(self.length, 0),
                                          Point(self.length, self.hole_width)],
                         "top_right": [Point(0, self.width - self.hole_width),
                                       Point(0, self.width),
                                       Point(self.hole_width, self.width)],
                         "top_middle": [Point(self.length / 2 - self.hole_width, self.width),
                                        Point(self.length / 2, self.width),
                                        Point(self.length / 2 + self.hole_width, self.width)],
                         "top_left": [Point(self.length - self.hole_width, self.width),
        ...
        for borders in holes_borders.values():
            plt.plot([borders[i].x for i in range(len(borders))],
                     [borders[i].y for i in range(len(borders))],
                     color='red')
